...
- If the bitmask bit is `1`, the corresponding memory address bit is _overwritten with 1_.
- If the bitmask bit is `X`, the corresponding memory address bit is _floating_.

A _floating_ bit is not connected to anything and instead fluctuates unpredictably. In practice, this means the floating bits will take on all _possible values_, potentially causing many memory addresses to be written all at once!
