...
So, in this example, `<em>5</em>` bricks can be safely disintegrated.
...
* Disintegrating brick `A` would cause all `<em>6</em>` other bricks to fall.
* Disintegrating brick `F` would cause only `<em>1</em>` other brick, `G`, to fall.

Disintegrating any other brick would cause _no other bricks_ to fall. So, in this example, the sum of _the number of other bricks that would fall_ as a result of disintegrating each brick is `<em>7</em>`.
