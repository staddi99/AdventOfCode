...
* `<em>9</em>` components: `cmg`, `frs`, `lhk`, `lsr`, `nvd`, `pzl`, `qnr`, `rsh`, and `rzs`.
* `<em>6</em>` components: `bvb`, `hfx`, `jqt`, `ntq`, `rhn`, and `xhk`.

Multiplying the sizes of these groups together produces `<em>54</em>`.
...
```
Power required is now <em class="star">49 stars</em>.
