...
So, there are `<em>13</em>` positions the tail visited at least once.
...
Now, you need to keep track of the positions the new tail, `9`, visits. In this example, the tail never moves, and so it only visits `<em>1</em>` position. However, _be careful_: more types of motion are possible than before, so you might want to visually compare your simulated rope to the one above.
...
Now, the tail (`9`) visits `<em>36</em>` positions (including `s`) at least once:
