...
const inputRun = (mode, memory, index, base) => (_input) => {
...
const command = (memory, index, base) => {
  ...
  switch (instruction) {
    ...
    case 3:
      return inputRun(mode, memory, index, base);
    case 4:
...
const Tuple = {
  ...
  mapFirst: (f, tuple) =>
    Tuple.pair(f(Tuple.first(tuple)), Tuple.second(tuple)),
  mapSecond: (f, tuple) =>
    Tuple.pair(Tuple.first(tuple), f(Tuple.second(tuple))),
  mapBoth: (f1, f2, tuple) =>
    Tuple.pair(f1(Tuple.first(tuple)), f2(Tuple.second(tuple))),
};
  ...
  while (map.unvisited.length > 0) {
    ...
    switch (status) {
      ...
      default:
        ...
        Grid.unvisited(map, unvisited).forEach((tuple) =>
          map.unvisited.push(tuple)
        );
        point = unvisited;
    }
  }
