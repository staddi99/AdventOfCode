...
class Vector {
  ...
  public hasPoint(point: Point): boolean {
    if (point.x < this.min_x || point.x > this.max_x || point.y < this.min_y || point.y > this.max_y) {
      return false;
    }
...
for (let i: number = 0; i < wire1.length; i++) {
  ...
  for (let j: number = 0; j < wire2.length; j++) {
    ...
    if (ip && v1.hasPoint(ip) && v2.hasPoint(ip)) {
      ...
      const wire1Length: number = wire1.slice(0, i).reduce((sum: number, v: Vector) => sum += v.length, 0);
      const wire2Length: number = wire2.slice(0, j).reduce((sum: number, v: Vector) => sum += v.length, 0);
      const path = wire1Length + wire2Length + getDistance(v1.p, ip) + getDistance(v2.p, ip);
